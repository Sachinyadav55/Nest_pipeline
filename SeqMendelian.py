...
class SeqCohort:
    ...
    def mendelian(self, vcffile):
        #Run mendelian module
        self.pattern = config['Mendelian']['pattern']
        self.thresh = config['Mendelian']['thresh']
        self.database = config['Mendelian']['database']
        self.vcffile = ranker(vcffile, self.outdir)
        outfile = caseselect(outfile, self.pattern, self.outdir, self.thresh)
        outfile = annovar(outfile, self.outdir)
        outfile = dcm(outfile, self.outdir, self.database)
        split(outfile, self.outdir) 
